...
# 主函数
# ============================
def generate_triples_with_cpm(sym_dir, herb_dir, pres_path,
                              fused_file,
                              presc_cpm_file,
    ...
    for i, (sym_set, herb_set) in enumerate(prescriptions):
        for sid in sym_set:
            ...
            if s_name not in entity_name2id:
                ...
                current_index += 1
            for hid in herb_set:
                h_name_raw = herb_entities[hid]
                # CHP 名称映射到主名
                h_name = alias2main.get(h_name_raw, h_name_raw)
        ...
        if pd.isna(cpm_id):
            continue
        if cpm_id not in entity_name2id:
            entity_name2id[cpm_id] = current_index
            entity_types[cpm_id] = "cpm"
            current_index += 1

        # symptom -> CPM
        for sid in sym_set:
            s_name = sym_entities[sid]
            triples.append((s_name, "关联CPM", cpm_id))
            triples.append((cpm_id, "关联症状", s_name))

        # CPM -> CHP
        chp_list_str = str(presc_row["CHPs_in_CPM"])
        chp_list = [alias2main.get(h.strip(), None) for h in chp_list_str.split(",")]
        chp_list = [h for h in chp_list if h is not None]
        for h_name in chp_list:
            if h_name not in entity_name2id:
                entity_name2id[h_name] = current_index
                entity_types[h_name] = "herb"
                current_index += 1
            triples.append((cpm_id, "包含", h_name))
            triples.append((h_name, "属于", cpm_id))

    # === 3️⃣ 症状 KG 三元组 ===
    for sid1, rid, sid2 in sym_edges:
        rel_name = sym_relations.get(rid, f"sym_rel_{rid}")
        rel_name_rev = rel_name + "_逆"
        h_name = sym_entities[sid1]
        t_name = sym_entities[sid2]
        triples.append((h_name, rel_name, t_name))
        triples.append((t_name, rel_name_rev, h_name))
        for n, sid in [(h_name, sid1), (t_name, sid2)]:
    ...
    for hid1, rid, hid2 in herb_edges:
        rel_name = herb_relations.get(rid, f"herb_rel_{rid}")
        rel_name_rev = rel_name + "_逆"
        h_name = herb_entities[hid1]
        t_name = herb_entities[hid2]
        triples.append((h_name, rel_name, t_name))
        triples.append((t_name, rel_name_rev, h_name))
        for n, hid in [(h_name, hid1), (t_name, hid2)]:
            if n not in entity_name2id:
                entity_name2id[n] = current_index
    ...
    with open(output_tsv, 'w', encoding='utf-8') as f:
        for h, r, t in triples:
            f.write(f"{h}\t{r}\t{t}\n")
    print(f"✅ 已生成 {len(triples)} 条三元组，保存为 {output_tsv}")

    # === 保存 entity_name2id.pkl ===
    with open("/home/gyj/local/SubgraphRAG-on-tcmmkg-main/entity_name2id.pkl", "wb") as f:
        pickle.dump(entity_name2id, f)
    print(f"✅ 已保存 entity_name2id.pkl，共 {len(entity_name2id)} 个节点映射")

    # === 保存 Excel 查看 (含 type 列) ===
    df = pd.DataFrame({
        "node_id": list(entity_name2id.keys()),
        "index": list(entity_name2id.values()),
        "type": [entity_types[n] for n in entity_name2id.keys()]
    })
    df.to_excel("/home/gyj/local/SubgraphRAG-on-tcmmkg-main/entity_mapping.xlsx", index=False)
    print(f"✅ 已保存 entity_mapping.xlsx")
    
        # === 保存 entity_identifiers.txt ===
    entity_id_path = "/home/gyj/local/SubgraphRAG-on-tcmmkg-main/retrieve/data_files/entity_identifiers.txt"
    with open(entity_id_path, "w", encoding="utf-8") as f:
        for name, idx in sorted(entity_name2id.items(), key=lambda x: x[1]):
           f.write(f"{name}\n")  # ✅ 只写实体名
    print(f"✅ 已保存 entity_identifiers.txt: {entity_id_path}")
    
    
    # === 保存 relation_list.txt ===
    relation_set = set(r for _, r, _ in triples)

    relation_id_path = "/home/gyj/local/SubgraphRAG-on-tcmmkg-main/retrieve/data_files/relation_list.txt"
    with open(relation_id_path, "w", encoding="utf-8") as f:
        for idx, rel in enumerate(sorted(relation_set)):
            f.write(f"{rel}\n")
    print(f"✅ 已保存 relation_list.txt: {relation_id_path}（共 {len(relation_set)} 个关系）")
...
if __name__ == "__main__":
    ...
    # pres_path="/home/gyj/local/SubgraphRAG-on-tcmmkg-main/try_train_can_delete.txt"
    ...
    generate_triples_with_cpm(sym_dir, herb_dir, pres_path,
                              fused_file,
                              presc_cpm_file,
                              output_tsv)
